...
#SBATCH --mem                   62G
#SBATCH --job-name              s-GSA-A
#SBATCH --output                log/s-GSA-A.%A_%a.out
#SBATCH --error                 log/s-GSA-A.%A_%a.err
...
# Run
python -u optimise-cc-single.py -d ${DESIGN} -l ${PATH2MODEL} -n ${N} -r ${SLURM_ARRAY_TASK_ID} -p -1 --tmp > ${LOG}/${DESIGN}_${MODEL}_n${N}_x${SLURM_ARRAY_TASK_ID}-z${Z}.log
