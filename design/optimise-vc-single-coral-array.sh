...
#SBATCH --mem                   62G
#SBATCH --job-name              sp-LSA-A
#SBATCH --output                log/sp-LSA-A.%A_%a.out
#SBATCH --error                 log/sp-LSA-A.%A_%a.err
...
# Run
Z=0.1
python -u optimise-vc-single.py -d ${DESIGN} -l ${PATH2MODEL} -n ${N} -r ${SLURM_ARRAY_TASK_ID} -z ${Z} -p -1 --tmp > ${LOG}/${DESIGN}_${MODEL}_n${N}_x${SLURM_ARRAY_TASK_ID}-z${Z}.log
