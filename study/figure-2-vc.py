...
for opt_model in opt_models:
    ...
    for opt_measure in opt_measures:
        # Load score
        score_file = './cross-criteria-evaluate-vc/score-%s-%s.txt' % \
...
# Benchmark
benchmark = []
benchmark_name = ['ch3', 'groenendaal-2015']
for b in benchmark_name:
    score_file = './cross-criteria-evaluate-vc/score-%s.txt' % b
    try:
        score = np.loadtxt(score_file)
    except:
        score = np.full((len(model_side), len(measure_side)), np.nan)
...
for i, name in enumerate(names):
    ...
    x = xi * (1 / 6.) + (1 / 6.) * 0.05
    y = (2 - yi) * (1 / 2.) - (1 / 2.) * 0.05
    color = 'white' if i in [2, 5, 7, 8, 11] else 'black'
