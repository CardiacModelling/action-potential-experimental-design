...
nx, ny = axes.shape
for i, k in enumerate(model_true._biomarkers.list_of_biomarkers):
    a, b = int(i / ny), i % ny
    xmin, xmax = np.min(data_list[:, i]), np.max(data_list[:, i])
    xmin = min(xmin, data_no_noise[i])
    xmax = max(xmax, data_no_noise[i])
    axes[a, b].hist(data_list[:, i], bins=np.linspace(xmin, xmax, 50))
...
plt.savefig('%s/fig4a.pdf' % (savedir), format='pdf', bbox_inches='tight')
plt.close()
