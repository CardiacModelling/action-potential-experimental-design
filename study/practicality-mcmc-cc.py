...
protocol = protocol.flatten().round()
# protocol = np.append([-80., 100.], protocol)  # Add 0.1s holding potential


# Create true model and synthetic data
model_true = method.model.CCModel(info.true_model_file, transform=None, dt=dt, n_steps=n_steps)
parameters_true = np.ones(model_true.n_parameters())
model_true.design(protocol)
...
model_fit = method.model.CCModel(info.fit_model_file, transform=None, dt=dt, n_steps=n_steps)
model_fit.design(protocol)
...
log_prior = pints.ComposedLogPrior(
    *([pints.LogNormalLogPrior(0, 0.15)] * problem.n_parameters())
)
...
for _ in range(3):
    assert(log_posterior(np.ones(model_fit.n_parameters())) ==\
            log_posterior(np.ones(model_fit.n_parameters())))
